...
const API_KEY = "7b8de563ef212093104de9b3654dc4d2"; // Paste your API here
...
function save(){
    let cityInputs = cityInput.value.trim();
    let cityStorage = JSON.parse(localStorage.getItem("cityStorage")) || [];
    cityStorage.push(cityInputs);
    localStorage.setItem("cityStorage", JSON.stringify(cityStorage));
    makeSaveShit(cityStorage)
}
function makeSaveShit(cityStorage){
   const historyContainer = document.querySelector(".historyContainer"); 
   historyContainer.innerHTML = "";
   cityStorage.forEach((city) => { 
    const historyButton = document.createElement("li");
    historyButton.textContent = city;
    historyButton.className += "shitHistory"
    historyContainer.appendChild(historyButton);


    historyButton.addEventListener("click", (event) => {
        event.preventDefault();
        let pastCityName = historyButton.textContent;
        getCityCoordinates(pastCityName);
    })
     })
}
